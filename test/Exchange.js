...
describe('Exchange', () => {
  ...
  beforeEach(async () => {
    ...
    token1 = await Token.deploy('THE GREEK AND GLOVER', 'PETE THE GREEK', '1000000')
    ...
    beforeEach(async () => {
      ...
      describe('Cancelling orders', async () =>{
        ...
        describe("Failure", async () => {
          ...
          it("rejects invalid order ids", async () => {
            // user1 deposits tokens
            const invalidOrderId = 9999 // false id
            await expect(exchange.connect(user1).cancelOrder(invalidOrderId)).to.be.reverted // if the user id is 9999, its invalid
          });
      ...
      describe('Fill orders', async () => {
        describe('Success', () =>{
          ...
          it('exectues the trade and charges a fee', async() =>{
            // token give
        ...
        describe('Failure', async() =>{
          ...
          it('rejects already filled orders', async() =>{
            ...
            await expect(exchange.connect(user2).fillOrder(1)).to.be.reverted // try to fill the same order twice
          })
          it('rejects canceled orders', async() =>{
            transaction = await exchange.connect(user1).cancelOrder(1)
            await transaction.wait()

            await expect(exchange.connect(user2).fillOrder(1)).to.be.reverted // try to fill cancled order
          })
        })
      })
  })
